...
import TourCard from "../../components/landing/tour/TourCard";
...
const TourPage = () => {
  ...
  // Filter and sort tours when dependencies change
  useEffect(() => {
    let filtered = [...tours];
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((tour) => {
        ...
        const country = (tour.country || "").toString().toLowerCase();
        const description = tourService.parseDescription(tour.description || "").toString().toLowerCase();
        ...
        //   highlights.includes(lowerSearch)
        );
      });
    }
    ...
    if (locationFilter) {
      filtered = filtered.filter(
        (tour) =>
          tour.country &&
          tour.country.toLowerCase().includes(locationFilter.toLowerCase())
      );
    }
    ...
    if (durationFilter) {
      filtered = applyDurationFilter(filtered, durationFilter);
    }
  ...
  const applySorting = (tourList, sortType) => {
    switch (sortType) {
      case "Name A-Z":
        return tourList.sort((a, b) => a.name.localeCompare(b.name));
      case "Name Z-A":
        return tourList.sort((a, b) => b.name.localeCompare(a.name));
  ...
  const applyBudgetFilter = (tourList, budgetRange) => {
    if (!budgetRange) return tourList;

    return tourList.filter((tour) => {
      const budget = tour.estimatedBudget || 0;
  ...
  const applyDurationFilter = (tourList, durationRange) => {
    if (!durationRange) return tourList;
    ...
    return tourList.filter((tour) => {
      ...
      switch (durationRange) {
        ...
        case "15-21":
          return duration >= 15 && duration <= 21;
        case "21+":
          return duration > 21;
        default:
          return true;
  ...
  const handleBookNow = (tourId) => {
    ...
    // Example: navigate(`/booking/tour/${tourId}`);
  };
  ...
  const handleSearch = async (searchValue) => {
    ...
    if (searchValue && searchValue.length > 2) {
      try {
        const searchResults = await tourService.searchTours(searchValue);
  ...
  const handleDurationFilter = (durationRange) => {
  ...
  const formatDurationDisplay = (durationRange) => {
    if (!durationRange) return "";
    if (durationRange === "21+") return "21+ days";
    return durationRange.replace("-", " - ") + " days";
  };
  ...
  const hasActiveFilters =
    categoryFilter || locationFilter || budgetFilter || durationFilter || searchTerm;

  if (loading) {
    return (
      <>
        <HeaderBanner title={"Our Tours"} />
        <div className="container mx-auto px-4 py-8">
          <div className="flex justify-center items-center h-64">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary-500"></div>
            <span className="ml-3 text-lg">Loading tours...</span>
          </div>
        </div>
      </>
    );
          ...
          <div className="flex-1">
            ...
            {hasActiveFilters && (
              <div className="flex flex-wrap items-center gap-2 mb-6 p-4 bg-primary-50 rounded-lg border border-primary-200">
                ...
                {durationFilter && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-primary-100 text-primary-700 rounded-full text-sm">
                    Duration: {formatDurationDisplay(durationFilter)}
                    <button
            ...
            {filteredTours.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl text-gray-600 mb-2">No tours found</h3>
                ...
                >
                  {currentTours.map((tour) => (
                    <TourCard
                      ...
                      onBookNow={() => handleBookNow(tour._id || tour.id)}
                    />
                  ))}
                </div>
