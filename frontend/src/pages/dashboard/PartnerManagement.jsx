...
import AddEditPartnerModal from "../../components/dashboard/partner/AddEditPartnerModal";
import DeletePartnerModal from "../../components/dashboard/partner/DeletePartnerModal";
import ViewPartnerModal from "../../components/dashboard/partner/ViewPartnerModal";
...
const PartnerManagement = () => {
  ...
  const fetchPartners = async (showRefreshLoader = false) => {
    ...
    try {
      ...
      if (showRefreshLoader) {
        showNotification("Partners refreshed successfully!");
      }
    } catch (error) {
  ...
  const handlePartnerSubmit = async (partnerData) => {
    ...
    try {
      if (selectedPartner) {
        ...
        setPartners((prev) =>
          prev.map((p) =>
            p.id === selectedPartner.id ? { ...p, ...updatedPartner } : p
          )
        );
        showNotification("Partner updated successfully!");
      } else {
        // Create new partner
        const newPartner = await partnerService.create(partnerData);
  ...
  // Handle partner deletion using the real service
  const handleConfirmDelete = async () => {
    if (!selectedPartner) return;

    setIsLoading(true);
    try {
      await partnerService.remove(selectedPartner.id);
      setPartners((prev) =>
        prev.filter((partner) => partner.id !== selectedPartner.id)
      );
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div
                    ...
                    style={{
                      display: partnerService.getImageUrl(partner)
                        ? "none"
                        : "flex",
                    }}
                  >
                ...
                {partner.phone && (
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span className="truncate">
                      {partnerService.formatPhone(partner.phone)}
                    </span>
                  </div>
                )}
                {partner.address && (
      ...
      <div className="h-full overflow-y-auto mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Manage your business partners and their details
          </p>
        </div>
        ...
        <AddEditPartnerModal
          ...
          onSubmit={handlePartnerSubmit} // <- This should always be handlePartnerSubmit
        />
