...
import { Search, Plus, Edit3, Trash2, MapPin, Image, Check, AlertTriangle, Eye, RefreshCw, ChevronLeft, ChevronRight, Calendar, Tag } from 'lucide-react';
...
const TourManagement = () => {
  ...
  const handleViewTour = (tour) => {
    if (!tour.id) return;
    navigate(`/dashboard/tour/${tour.id}`);
    setIsViewModalOpen(true);
  };

  // Handle form submission for both create and update
  const handleTourSubmit = async (tourData, mainPhoto, galleryFiles) => {
    setIsLoading(true);
    try {
      // Validate images before submission
      const imagesToValidate = mainPhoto ? [mainPhoto, ...galleryFiles] : galleryFiles;
      if (imagesToValidate && imagesToValidate.length > 0) {
        if (tourService.validateImages) {
          const validation = tourService.validateImages(imagesToValidate);
      ...
      if (selectedTour) {
        ...
        await tourService.updateTour(selectedTour.id, data, mainPhoto, galleryFiles, tourData.keepGalleryImages || []);
        ...
        await tourService.createTour(data, mainPhoto, galleryFiles);
        showNotification('Tour added successfully!');
      }
      ...
      showNotification(`Failed to save tour: ${error.message}`, 'error');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getMainImage = (tour) => {
    if (tour.mainPhotoUrl) {
      return tourService.getFullImageUrl ? tourService.getFullImageUrl(tour.mainPhotoUrl) : tour.mainPhotoUrl;
    }
    return null;
  };
