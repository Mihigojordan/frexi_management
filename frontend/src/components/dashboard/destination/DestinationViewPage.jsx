...
const DestinationViewPage = () => {
  ...
  useEffect(() => {
    const fetchDestination = async () => {
      try {
        ...
        console.log('destinatins :' ,destinationData)
  ...
  const getAllImages = () => {
    ...
    if (destination.mainPhotoUrl) {
      images.push(destinationService.getFullImageUrl(destination.mainPhotoUrl));
    }
    
    const gallery = parseJsonField(destination.gallery);
    console.log(gallery);
    
    if (gallery && Array.isArray(gallery)) {
      gallery.forEach(img => {
        images.push(destinationService.getFullImageUrl(img));
      });
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary-50 to-primary-100 flex items-center justify-center">
        <div className="bg-white rounded-lg p-8 shadow-lg">
          ...
          <p className="mt-4 text-primary-600 text-center">Loading destination details...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-primary-50 to-primary-100 flex items-center justify-center">
        <div className="bg-white rounded-lg p-8 shadow-lg max-w-md w-full mx-4">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-primary-800 mb-2">Error Loading Destination</h2>
  ...
  if (!destination) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-primary-50 to-primary-100 flex items-center justify-center">
        <div className="bg-white rounded-lg p-8 shadow-lg">
          <p className="text-primary-600">Destination not found</p>
        </div>
      </div>
    );
  }
  ...
  const images = getAllImages();
  const description = destinationService.parseDescription(destination.description);
  const popularSites = parseJsonField(destination.popularSites);
  const highlights = parseJsonField(destination.highlights);
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-50 to-primary-100">
      ...
      <div className="bg-white shadow-sm border-b border-primary-100">
        <div className="   mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <h1 className="text-3xl font-bold text-primary-800">{destination.name}</h1>
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold text-primary-800">
                      {destination.estimatedBudget ? formatCurrency(destination.estimatedBudget) : 'Contact for pricing'}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white rounded-lg p-4 shadow-sm border border-primary-100">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-semibold text-primary-800">
                        {destination.areaKm2.toLocaleString()} km²
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                {destination.language && (
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <span className="text-primary-800 font-medium">{destination.language}</span>
                    </div>
                  </div>
                )}
                ...
                {destination.currencyUsed && (
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <span className="text-primary-800 font-medium">{destination.currencyUsed}</span>
                    </div>
                  </div>
                )}
                ...
                {destination.visaRequirements && (
                  <div className="bg-primary-50 rounded-lg p-4">
                    ...
                    <p className="text-primary-700 text-sm">{destination.visaRequirements}</p>
                  </div>
                )}
              </div>
            </div>
